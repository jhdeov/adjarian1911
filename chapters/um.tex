...
Word-finally in some words, it is lost (Table \ref{tab:Yerevan:SoundChange:Consonant:Gh}). However, the word `place' CA /teɫ/ <\armenian{տեղ}> on its own did not undergo this rule.\footnote{\translatorHD{For `yonder,' the SEA form shows regressive devoicing of /ɑjd-teʁ/ to /ɑjt-teʁ/. We don't know if CA also had regressive devoicing. For the question word `where,' stress is variable in SEA.}}
...
\translatorHD{In Yerevan, many other tenses seem to be built on the subjunctive (Table \ref{tab:Yerevan:morpho:verb:paradigm:complexSubjunctive}). The future and past future are built by adding the prefix /kə/ before the subjunctive present and subjunctive past. The debitive and debitive past are formed also by adding the proclitic /pti/ before the appropriate subjunctive form. SEA behaves essentially the same and I don't provide its paradigm.}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Yerevan} & \multicolumn{2}{l|}{cf. SEA `like!'} & \\
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Yerevan}& \multicolumn{2}{l|}{cf. SEA}
		&\\ 
			\hline	{\infgloss} & & & χos-e-l & \armenian{խոսել} & $\sqrt{}$-{\thgloss}-{\infgloss}\\
		& & & \multicolumn{2}{l|}{`to speak'} & \\
		\hline		{\prs} 1{\sg} & χos-um e-m & \armenian{խօսում էմ} &χos-um e-m & \armenian{խոսում եմ} & $\sqrt{}$-{\impfcvb} {\aux}-1{\sg} \\
				& & & \multicolumn{2}{l|}{`I speak'} & \\
		\hline		{\pst} {\impf} 1{\sg} & χos-um $\emptyset$-i-$\emptyset$  & \armenian{խօսում ի} & χos-um ej-i-$\emptyset$ & \armenian{խոսում էի}& $\sqrt{}$-{\impfcvb} {\aux}-{\pst}-1{\sg} \\
			& & & \multicolumn{2}{l|}{`I was speaking'} & \\
			\hline	{\pst} {\perfective} 1{\sg} & χos-e-t͡sʰ-i-$\emptyset$& \armenian{խօսէցի} & χos-e-t͡sʰ-i-$\emptyset$ & \armenian{խոսեցի} & $\sqrt{}$-{\thgloss}-{\aor}-{\pst}-1{\sg} \\
			& & & \multicolumn{2}{l|}{`I spoke'} & \\
			\hline	{\imp} 2{\sg} & χ\'os-i & \armenian{խօ՛սի} &χos-\'iɾ & \armenian{խոսի՛ր} & $\sqrt{}$-{\imp}.2{\sg}\\
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Yerevan}& \multicolumn{2}{l|}{cf. SEA}
		& \\ 
			\hline	{\infgloss} & & & hɑz-ɑ-l & \armenian{հազալ}& $\sqrt{}$-{\thgloss}-{\infgloss} \\
			& & & \multicolumn{2}{l|}{`to cough'} & \\
			\hline	{\prs} 1{\sg} & hɑz-əm e-m & \armenian{հազըմ էմ} &hɑz-um e-m & \armenian{հազում եմ}& $\sqrt{}$-{\impfcvb} {\aux}-1{\sg} \\
				& & & \multicolumn{2}{l|}{`I cough'} & \\
				\hline	{\pst} 		{\impf} 1{\sg} & hɑz-əm $\emptyset$-i-$\emptyset$&\armenian{հազըմ ի} & hɑz-um ej-i & \armenian{հազում էի}& $\sqrt{}$-{\impfcvb} {\aux}-{\pst}-1{\sg} \\
				& & & \multicolumn{2}{l|}{`I was coughing'} & \\			\hline{\pst} {\perfective} 1{\sg} & hɑz-ɑ-t͡sʰ-i-$\emptyset$& \armenian{հազացի} & hɑz-ɑ-t͡sʰ-i-$\emptyset$ & \armenian{հազեցի} & $\sqrt{}$-{\thgloss}-{\aor}-{\pst}-1{\sg} \\
				& & & \multicolumn{2}{l|}{`I coughed'} & \\	
						\hline {\prs} {\perfect} 1{\sg} & hɑz-ɑ-t͡sʰ-el e-m & \armenian{հազացէլ էմ} & hɑz-ɑ-t͡sʰ-el e-m & \armenian{հազացել եմ} & $\sqrt{}$-{\thgloss}-{\aor}-{\perfcvb} {\aux}-1{\sg} \\
				& & & \multicolumn{2}{l|}{`I have coughed'} & \\	
						\hline{\imp} 2{\sg} & hɑz-\'ɑ & \armenian{հազա} &hɑz-\'ɑ & \armenian{հազա՛} & $\sqrt{}$-{\thgloss} \\
				& & & \multicolumn{2}{l|}{`cough!'} & \\	\hline 
	\end{tabular}
}
\end{table}
...
... it is the verb which is inflected, while the /pti/ <\armenian{պտի}> doesn't change. The second form is rare in other places. It does not exist at all in the /kə/ <\armenian{կը}> branch. 
...
The forms /m\'ekʰ siɾil/ <\armenian{մէ՛ք սիրիլ}> `don't like' or the inverse order /s\'iɾil mekʰ/ <\armenian{սի՛րիլ մէք}> (where the plural marker of the verb has passed onto the particle) are also used in the Karabakh dialect.
...
\translatorHD{Contrast these irregular verbs with /-is/ against regular verbs with /-um/ in Table \ref{tab:Yerevan:morpho:verb:paradigm:presentIndc}. Such irregular verbs are monosyllabic in the infinitive like SEA /ɡ-ɑ-l/ `to give'.}
...
\translatorHD{This process is also described for Iranian Armenian as the same process for the perfective converb \citep[\S3.3]{DolatianEtAl-prep-IranianGrammar}.} 
...
\begin{table}[H]
	...
	\hline (?) & t͡ʃʰ-\'e-m uz-il & \armenian{չէ՛մ ուզիլ} & & \\
...
After vowel-final words, the definite article (\armenian{դիմորոշ յօդը}) is repeated or doubled. 
...
For monosyllabic verbs, the stem of the present and imperfective uses /-um/ <\armenian{ում}> instead of /-is/ <\armenian{իս}>, thus fully regular (Table \ref{tab:Yerevan:subdialect:bayazit:imperfIrregIs}). 

\translatorHD{Contrast against Table \ref{tab:Yerevan:morpho:verb:other:imperfIrregIs}.}
...
The past participle is also formed with the formative /-uk/ <\armenian{ուկ}>, but only in passives (\armenian{կրաւորական}) and middle verbs (\armenian{չէզոք}). 

\translatorHD{I think he means for intransitives in general.}
...
The past perfective gets the ending /m/ <\armenian{մ}> (Table \ref{tab:Yerevan:subdialect:Astabad:pstm}).
 ...
 \translatorHD{He means this subdialect uses the suffix /-m/ as a past perfective 1{\sg} marker, whereas Yerevan and SEA use a zero suffix.}
...
When the words for `horse' and `egg' take the definite suffix, the forms are as in Table \ref{tab:Yerevan:subdialect:tabriz:def}.
 ...
 \translatorHD{He means that the definite suffix is /-jə/ for these forms instead of /-n/. Note that suffix marks definiteness in the modern vernaculars but it's a distal deixis marker in CA.}
...
\translatorHD{There doesn't to be much work on the old dialect of Yerevan, before Yerevan Armenians switched to using SEA. Two sources that I know are \citep[25]{Patkanian-1869-RussianDialects} and \citet[164-180]{Gharibyan-1941-SummaryArmenianDialectology}.  For the Bayazit vernacular, see \citet[226]{Martirosyan-2019-ArmenianDialectsBigVersionRussianJournal}.}
...
The consonants /b ɡ d d͡z d͡ʒ/ <\armenian{բ գ դ ձ ջ}> are pronounced with   perfect voicing, and with much more voicing than in the Yerevan dialect, like the French <b, g, d> sounds. 
...
The following is the declension of the word /div/ <\armenian{դիվ}> from Classical /deu̯/ <\armenian{դեւ}> `demon'. 

\translatorHD{I suspect that the final /n/ in all the words in Table \ref{tab:Tbilisi:morpho:noun} is actually a separate definite suffix /-n/, but I'm not sure. But we can't be sure without more data.  (?)}
...
The verbs are conjuated in the manner of Yerevan, except for the required phonetic changes. For example, for the copula verb in the present, the Classical sounds /e, ē/ <\armenian{ե, է}> become   /i/ <\armenian{ի}> for all   persons (except for the third). And because of this, the stem of the verb uses the endings /-um im, -um is/ <-\armenian{ում իմ}, -\armenian{ում իս}>. 
...
In the imperfective, the reflex of the Classical sound /ē/ <\armenian{է}> does not shorten. The forms are pronounced as in Old Armenian  (Table \ref{tab:Tbilisi:morpho:verb:pastaux}, \ref{tab:Tbilisi:morpho:verb:imperf}). 

\translatorHD{What he means is that unlike in the Yerevan dialect, the auxiliary /e/ and theme vowel /e/ do not delete before the past suffix /-i/. The Tbilisi dialect thus patterns with SEA in this regard.} 
...
It is also voiced as in Table \ref{tab:Tbilisi:morpho:verb:futKgu}.
 ...
 \translatorHD{Note that he doesn't state what is the root of these verbs. So I segment the prefix as /ɡu/ based on the contrast against SEA, but Adjarian might have meant that the prefix as /ɡ-/ while the verb was /ukʰɑm/. (?)}
...
\translatorHD{In Tbilisi, many other tenses seem to be built on the subjunctive (Table \ref{tab:Tbilisi:morpho:verb:paradigm:complexSubjunctive}). The future and past future are built by adding the prefix /ku/ before the subjunctive present and subjunctive past. The debitive and debitive past are formed also by adding the proclitic /piti/ before the appropriate subjunctive form. I don't provide morpheme glosses for these forms for space. SEA behaves essentially the same (with the expected difference in theme vowels) and I don't provide its paradigm.}
...
\translatorHD{Adjarian provides a paradigm for something he calls the `debitive past perfect'. It consists of the debitive particle /piti/, plus what appears to be the perfective converb with /-il/, and then the past auxiliary. He does not explain what this construction is supposed to mean.}


\begin{table}[H]
	\centering
	\caption{Debitive past perfect  <\armenian{պարտաւորական գերակատար}> for the verb `to like' in the Tbilisi dialect}
...
\begin{table}[H]
	...
	\hline & \multicolumn{2}{l|}{Tbilisi} & \multicolumn{2}{l|}{cf. SEA `like!' and `do not like!'} & \\\hline
	...
	& & & siɾ-ekʰ & \armenian{սիրեք}   & $\sqrt{}$-{\imp}.2{\pl} \\ \hline 
...
& & 		& \multicolumn{2}{l|}{`live!'}\\	\hline 	Prohibitive & \multicolumn{2}{l|}{{\proh} $\sqrt{}$-{\thgloss}(?)-{\agr}} & \multicolumn{2}{l|}{{\proh} $\sqrt{}$-{\thgloss}-{\agr}}\\
...
\translatorHD{Another class is the irregular infixed verbs that end in the morph sequence /-n-i-l/. The /n/ is synchronically a meaningless stem-extender that's deleted in the past perfective. In SEA, the theme vowel /i/ is replaced by /e/. We show just the Tbilisi and SWA paradigms for illustration (Table \ref{tab:Tbilisi:morpho:verb:otherClass:aorist:infix}).}
...
\translatorHD{He means that these words are pronounced as in SEA with a /əv/ sequence instead of /u̯ɑ/.}\footnote{\translatorHD{I couldn't unambiguously track down what the word <\armenian{թթուեալ}> meant, so I couldn't determine its SEA reflex.}}
...
\paragraph{Voicing assimilation between dorsal fricatives and stops/affricates} 

The Classical sounds /χ, ɫ/ <\armenian{խ, ղ}> merge with the following stop or affricate to form a /χ/ <\armenian{խ}> + voiceless sequence (Table \ref{tab:Karabakh:phonology:soundChange:cons:velarassimilation}). 
...
This ending is lost in almost all our dialects (\translatorHD{such as in SEA}). This form become /nə/ <\armenian{նը}> in Karabakh, creating a unique characteristic for this dialect.
...
Almost all the nominal case markers can be preceded by the additional units /-ɑn, -nɑn/ <\armenian{ան, նան}> (Table \ref{tab:Karabakh:morpho:noun:extender}).
 ...
 \translatorHD{It seems these morphs act as stem-extenders between stems and case suffixes. But I don't gloss them as {\nx} because we can't be sure given the limited data.  It's also  unclear if they're optional.}
...
\translatorHD{For the third person personal pronouns, SEA uses two sets of pronouns: intensive or empatic  pronouns like /iŋkʰə/, and a non-intensive or non-emphatic pronoun that's connected to the distal demonstrative /nɑ/. The intensive pronouns are in Table \ref{tab:Karabakh:morpho:pronoun:log}. The term `intensive' is from   \citet[126]{DumTragut-2009-ArmenianReferenceGrammar}, and `emphatic' from \citet{Donabedian-2018-WestArmTypoSocio}.}


\begin{table}[H]
	\caption{Inflection paradigm for third person intensive pronouns in the Karabakh dialect}\label{tab:Karabakh:morpho:pronoun:log}
	\centering 
	\begin{tabular}{| l| llll|}
		\hline & 3SG & & 3PL & \\
		& `he/she' & & `they'& \\
...
\translatorHD{Adjarian shows the paradigm for the verb `to like,' as a reflex from Classical /siɾ-e-l/ <\armenian{սիրել}>.} 
...
\translatorHD{In Karabakh, many other tenses seem to be built on the subjunctive (Table \ref{tab:Karabakh:morpho:verb:paradigm:complexSubjunctive}). The future and past future are built by adding the prefix /kə/ before the subjunctive present and subjunctive past. (Note that this prefix is /kʰə/ before voiceless sounds, as stated by Adjarian (\S\ref{sec:Karabakh:morpho:verb:overview}). SEA behaves essentially the same and I don't provide its paradigm.}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Karabakh} & \multicolumn{2}{l|}{cf. SEA `like!'} & \\
		2SG & s\'iɾ-i & \armenian{սի՛րի} & siɾ-\'iɾ & \armenian{սիրի՛ր} & $\sqrt{}$-{\imp}.2{\sg}
		\\
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{|l|ll|ll|ll|}
		\hline & \multicolumn{4}{l|}{Karabakh} & \multicolumn{2}{l|}{cf. SEA `do not like!'} \\
...
\begin{table}[H]
	...
	\label{tab:Karabakh:morpho:verb:paradigm:presentpastPerfect}
		\resizebox{\textwidth}{!}{%
		\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Karabakh} & \multicolumn{2}{l|}{cf. SEA `I have/had liked'} & \\
...
\translatorHD{Adjarian likewise mentions the definite future and the definite past future. They're formed by taking the future participle and adding the present or past auxiliaries. He does not explain what a definite future means however.}\footnote{\translatorHD{Adjarian does not use the phrase `definite future' elsewhere in the book. Hrach Martirosyan informs me that he likewise does not know this phrase. A grammar of Karabakh Armenian just calls this construction a simple future \citep[149]{Davtyan-1966-KarabakhDialectMap}.}}
...
\translatorHD{In the intensive mood (\armenian{սաստկական եղանակ}), Adjarian briefly illustrates 4 possible systems. These systems are formed by taking a finite or non-finite form. Adjarian doesn't explain the structure or meaning of such systems. I conjecture the following rules in (\ref{sent:Karabakh:morpho:verb:intensive}).}
...
\translatorHD{In the immediate mood (\armenian{անմիջական եղանակ}), Adjarian briefly illustrates 4 possible systems. Adjarian doesn't explain the structure or meaning of such systems. I conjecture that the morphological strategy is to take the instrumental form of the verb (suffixed with /-ɑv/), and then use combinations of auxiliaries and narrative formatives. (\ref{sent:Karabakh:morpho:verb:Immediate}).}
...
The debitive form (1SG /piti-m/ <\armenian{պիտիմ}>, 2SG /piti-s/ <\armenian{պիտիս}>) is shortened to /dem, des, den/ <\armenian{դէմ, դէս, դէն}> (\ref{sent:Karabakh:subdialct:Gazakh:deb}).\footnote{\translatorHD{Adjarian didn't provide a translation for the below sentences, and I found them hard to guess. The first sentence's translation is my guess. Vahagn Petrosyan suggested the translation for the second sentence. Without an explicit translation by Adjarian or more data from this dialect, I'm not completely confident in the glossing and translation.}}
...
\paragraph{Vowel lengthening of /o/} \label{section:shamakhi:phono:change:o:lengthening}

The Shamakhi dialect has a special sound change where Classical /ɑu̯/ <\armenian{աւ}> becomes /o/ <\armenian{օ}> before the Classical vowels /o, ɑu̯/ <\armenian{ո, աւ}> (Table \ref{tab:Shamakhi:phonology:soundChange:vowel:olength}). 

\translatorHD{To clarify, he means that if the Classical form had a substring <\armenian{աւո}> /ɑ{wo}/ (which he treats as including a diphthong /ɑu̯o/), this string became /oo/ in Shamakhi. In contrast in SEA, such a string became /ɑvo/. I suspect his transcription <\armenian{օօ}> /oo/ signifies a long vowel /oː/ but I'm not sure. (?)}


It becomes /œo/ <\armenian{էօօ}> when next to soft vowels. 

\translatorHD{I assume his transcription <\armenian{էօօ}> /eoo/ is actually /œo/. I suspect this is a diphthong but I'm unsure.}
...
\translatorHD{I don't see how Adjarian's examples relate to his proposed process of schwa epenthesis and vowel harmony.}
...
\translatorHD{Note that these multi-word phrases likely didn't exist in Classical Armenian, so we can assume that Adjarian was treating the shared closest ancestor of Shamakhi and SEA as being an SEA-like variety.}
...
The same process occurs next to the possessive suffixes (\armenian{դիմորոշներուն քով}) from CA /-s, -d, -n/ <\armenian{ս, դ, ն}> (Table \ref{tab:Shamakhi:morpho:noun:article:poss}).
 ...
 \translatorHD{He means that we also find harmony for the schwa that is epenthesized between a stem-final consonant and a possessive suffix.}
...
The pronouns also have another interesting form which is unique to this dialect. Here, this is the addition of formative /-ik/ <\armenian{իկ}> . Although this formative appears in other dialects and in Classical Armenian,   there it is only added for the demonstratives (Table \ref{tab:Shamakhi:morpho:pronoun:ClasicalDem}a) and it is not declined (Table \ref{tab:Shamakhi:morpho:pronoun:ClasicalDem}b).
 ...
 \translatorHD{Note that Adjarian isn't clear on which Armenian varieties have the demonstratives in Table \ref{tab:Shamakhi:morpho:pronoun:ClasicalDem}. I had to personally catalog them and track down their attestations.}
...
The causative (\armenian{անցողական}) formative is /-t͡sʰu/ <\armenian{ցու}> (Table \ref{tab:Shamakhi:morpho:verb:causative}).
 ...
 \translatorHD{Unfortunately, Adjarian doesn't translate his examples to SEA or SWA, so I can only guess what they're supposed to mean or how they should be segmented based on the SEA forms that resemble them the most.} 
...
After soft vowels (\translatorHD{I think he means after front vowels (?)}), the sound /ɑ/ <\armenian{ա}> becomes /æ/ <\armenian{ա̈} > (Table \ref{tab:Shamakhi:morpho:verb:soft}).
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|l|}
		\hline & \multicolumn{2}{l|}{Shamakhi} & \multicolumn{2}{l|}{cf. SEA`like!'} & \\
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|}
		\hline & \multicolumn{2}{l|}{Shamakhi} & \multicolumn{2}{l|}{cf. SEA `do not like!'} \\
...
The pronouns almost all follow the Yerevan system (Table \ref{tab:Astrakhan:morphology:pronoun:sample}).
 ...
 \translatorHD{Adjarian doesn't translate these pronouns; so I'm guessing what they are.}
...
The Karabakh-style forms are used (\ref{sent:Astrakhan:morpho:verb:presIndcKara}).
 ...
 \translatorHD{It's not clear to me how this is like Karabakh. I cannot tell what the /-m/ suffix should mean. The only  /-m/ suffix that I know of in Armenian i the present 1SG marker. But the following sentences are in the third person. It's possible that the /-m/ here is some non-finite suffix. (?)}
...
The past participle ends in /-el/ <\armenian{էլ}>, and in this way it forms the present perfect and past perfect (\armenian{յարակատարն ու գերակատարը}) in (\ref{sent:Astrakhan:morpho:verb:perfectivePart}).
 ...
 \translatorHD{Adjarian did not provide translations or reflexes, so I had to guess their meanings. (?)}
...
The ablative is formed with the formative /-e/ <\armenian{է}>. But for   words with the rhyme /-u/ <\armenian{ու}>, the ablative uses the formatives /-it͡sʰ, ut͡sʰ/, <\armenian{ից, ուց}> (Table \ref{tab:Julfa:morphology:noun:abl}).\footnote{\translatorHD{For the word /ɑrv-it͡sʰ/, Adjarian provides a translation <\armenian{յառուէ}>, but I couldn't figure out what this word meant. (?)}}
...
Some interesting forms are in Table \ref{tab:Julfa:morphology:pronoun:other}.\footnote{\translatorHD{He doesn't say what's interesting about them. (?)}}
...
For the present tense form of the copular verb, the Classical sound /e/ <\armenian{ե}>   of all the persons has changed to /ɑ/ <\armenian{ա}> 
...
The following are the primary tenses of the reflex of the Classical verb /ɡən-ɑ-l/ <\armenian{գնալ}> `to go'.  
...
\translatorHD{In Julfa, the future and past future are built on the subjunctive by adding the prefix /kə/ <\armenian{կը}> (Table \ref{tab:Julfa:morpho:verb:paradigm:complexSubjunctive}). SEA behaves essentially the same and I don't provide its paradigm.}
...
\paragraph{Change from Classical ending /n/ <\armenian{ն}>}\label{section:Aguli:phono:change:cons:n}
...
\translatorHD{To clarify, he means the word-final segment /n/.}
...
\translatorHD{It's not clear to me what this formative is supposed to be . It could be a meaningless stem-extender, as found in SEA pronouns: `from me' /ind͡z-ɑn-it͡sʰ/ glossed as 1{\sg}.{\dat}-{\nx}-{\abl}. Or it could be an oblique marker like the ones used in irregular SEA kinship words: `from a friend' /əŋkeɾ-ot͡ʃʰ-it͡sʰ/ glossed as `friend-{\obl}-{\abl}.  He doesn't cite any words with this infix, so it's unclear what should be the right gloss (?)}
...
The   imperfective has been lost; in its place, the dialect has invented a new strategy (Table \ref{tab:Agulis:morpho:verb:copulaPast}).
 ...
 \translatorHD{Whereas CA and SEA have a suffix /-i/ to encode past tense, Agulis instead uses /nel/ as a particle that marks the past.}
...
As can be seen, this new strategy for the past auxiliary is a reduced and shortened form of the Classical phrase /eɫe̯ɑl em/ <\armenian{եղեալ եմ}> or /le̯ɑl em/ <\armenian{լեալ եմ}> `I have been'. 

\translatorHD{Note that this Classical phrase consists of the participle /eɫe̯ɑl/ <\armenian{եղեալ}> of the verb `to be' /linel/ <\armenian{լինել}>, plus the copula as an auxiliary. Though it is unclear how a nasal was inserted from CA /eɫe̯ɑl/ to Agulis /nel/. It's possible that the nasal reflects a reduction from th stem /lin-/ of the infinitve /lin-e-l/ `to be.'}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|l|ll|ll|}
			\hline & \multicolumn{2}{l|}{Agulis} & \multicolumn{2}{l|}{cf. SWA}   \\ \hline 
...
\translatorHD{For contrast, Table \ref{tab:Agulis:morpho:verb:paradigm:presentPerfectMore sea} shows the 1SG forms compared with SEA as well.}
...
\translatorHD{For contrast, Table \ref{tab:Agulis:morpho:verb:paradigm:pastPerfect sea} shows the 1SG forms compared with SEA as well.}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|ll|ll|ll| l l|}
			...
			\hline   Agulis & Past perfective& kətɾ-e-l  & \armenian{կտրէլ}  & \'ɑh-ɑ-l & \armenian{ա՛հալ}  & hr-\'æ-h-æ-l & \armenian{հռա̈՛հա̈լ} \\
			SWA  & N/A&  N/A& & N/A & & N/A &   \\
...
\paragraph{Future and past future} \label{section:agulis:morph:verb:paradigm:future}
...
\translatorHD{Adjarian calls this the past future. It is thus implied that the 1SG form for `to cut' would mean ` was going to cut' in Agulis. However, Adjarian is implicitly comparing this construction against the SEA form which uses the particle /kə/. Yet the use of the future prefix /kə/ with past tense agreement would create a meaning like `I would cut' in SEA.}
...
\begin{table}[H]
	...
	\hline & \multicolumn{2}{l|}{Agulis} & \multicolumn{2}{l|}{cf. SEA} \\
\hline	2SG   `cut!' & ktɾ-\'e-$\emptyset$  & \armenian{կտրէ՛} & kətɾ-iɾ & \armenian{կտրի՛ր} \\
	& \multicolumn{2}{l|}{$\sqrt{}$-{\thgloss}-{\imp}.2{\sg}} & \multicolumn{2}{l|}{$\sqrt{}$-{\imp}.2{\sg}} \\
\hline	2SG   `forget!' & mr-\'æ-h-i & \armenian{մռա̈՛հի} & mor-ɑ-t͡sʰ-iɾ &\armenian{մոռացի՛ր}\\
	& \multicolumn{2}{l|}{$\sqrt{}$-{\lvgloss}-{\aor}-{\imp}.2{\sg}}  & \multicolumn{2}{l|}{$\sqrt{}$-{\lvgloss}-{\aor}-{\imp}.2{\sg}} \\
\hline	2SG   unclear verb & tʰ\'ɑk-ɑ-$\emptyset$& \armenian{թա՛կա} & &   \\
...
\begin{table}[H]
	...
	\begin{tabular}{|l|ll|ll|}
		\hline & \multicolumn{2}{l|}{Agulis} & \multicolumn{2}{l|}{cf. SEA} \\
		\hline 2SG   `do not tie!' (?)  & k\'ɑp-i-l m\'æ & \armenian{կա՛պիլ մա̈} & m\'i kɑp-iɾ & \armenian{մի՛ կապիր}\\ 
		& k\'ɑp-i m\'æ & \armenian{կա՛պի մա̈} & & \\ 
		& \multicolumn{2}{l|}{$\sqrt{}$-{\thgloss}-{\infgloss} {\proh}} & \multicolumn{2}{l|}{{\proh} $\sqrt{}$-{\imp}.2{\sg}}\\
		\hline 2SG   `do not say!' & n-\'ɑh-i-l m\'æ & \armenian{նա՛հիլ մա̈} & m\'i ɑs-ɑ-$\emptyset$ & \armenian{մի՛ ասա} \\
		& n-\'ɑh-i m\'æ & \armenian{նա՛հի մա̈} & & \\ 
		& \multicolumn{2}{l|}{?-$\sqrt{}$-{\thgloss}-{\infgloss} {\proh}} & \multicolumn{2}{l|}{{\proh} $\sqrt{}$-{\thgloss}-{\imp}.2{\sg}}\\
		\hline 2SG   `do not go away!' & hr-\'æ-n-i-l m\'æ & \armenian{հռա̈՛նիլ մա̈} & m\'i her-ɑ-t͡sʰ-iɾ & \armenian{մի՛ հեռացիր} \\
